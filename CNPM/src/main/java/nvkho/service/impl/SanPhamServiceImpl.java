...
public class SanPhamServiceImpl implements SanPhamService {
    ...
    public SanPhamResponse createSanPham(SanPhamRequest request, MultipartFile file) {
        if (sanPhamRepository.findByTenSP(request.getTenSP()).isPresent()) {
    ...
    public SanPhamResponse updateSanPham(String maSP, SanPhamRequest request, MultipartFile file) {
        ...
        Optional<SanPham> existingSanPham = sanPhamRepository.findByTenSP(request.getTenSP());
        if (existingSanPham.isPresent() && !existingSanPham.get().getMaSP().equals(maSP)) {
            throw new BadRequestException("Tên sản phẩm đã tồn tại");
        }
